...
/*************************************************************************************************/
unsigned char CH376WriteReqBlock(unsigned char *buf)
{
 unsigned char s,l;
...
//向USB主机端点的发送缓冲区写入数据块
/*************************************************************************************************/
void CH376WriteHostBlock(unsigned char *buf,unsigned char len)
{
...
//向内部缓冲区指定偏移地址写入数据块
/*************************************************************************************************/
void CH376WriteOfsBlock(unsigned char *buf,unsigned char ofs,unsigned char len)
{
...
//设置将要操作的文件的文件名
/*************************************************************************************************/
void CH376SetFileName(unsigned char *name)
{
/*    unsigned char    i;*/
 unsigned char c;
...
/*************************************************************************************************/
unsigned int CH376Read32bitDat(void)
{
 ...
 c3 = xReadCH376Data();
 xEndCH376Cmd( );
 return(c0 | (unsigned short)c1 << 8 | (unsigned int)c2 << 16 | (unsigned int)c3 << 24);
}

/*************************************************************************************************/
//读CH376芯片内部的8位变量
/*************************************************************************************************/
unsigned char CH376ReadVar8(unsigned char var)
{
 unsigned char c0;
...
//写CH376芯片内部的8位变量
/*************************************************************************************************/
void CH376WriteVar8(unsigned char var,unsigned char dat)
{
...
//读CH376芯片内部的32位变量
/*************************************************************************************************/
unsigned int CH376ReadVar32(unsigned char var)
{
...
//写CH376芯片内部的32位变量
/*************************************************************************************************/
void CH376WriteVar32(unsigned char var,unsigned int dat)
{
 xWriteCH376Cmd(CMD50_WRITE_VAR32);
 xWriteCH376Data(var);
 xWriteCH376Data((unsigned char)dat );
 xWriteCH376Data((unsigned char)( (unsigned short)dat >> 8 ));
 xWriteCH376Data((unsigned char)( dat >> 16 ));
 xWriteCH376Data((unsigned char)( dat >> 24 ));
 xEndCH376Cmd();
}
...
//获取磁盘和文件系统的工作状态
/*************************************************************************************************/
unsigned char CH376GetDiskStatus(void)
{
...
unsigned char Wait376Interrupt(void)
{
...
//发出命令码后,等待中断
/*************************************************************************************************/
unsigned char CH376SendCmdWaitInt(unsigned char mCmd)
{
 xWriteCH376Cmd(mCmd);
 xEndCH376Cmd();
...
//发出命令码和一字节数据后,等待中断
/*************************************************************************************************/
unsigned char CH376SendCmdDatWaitInt(unsigned char mCmd,unsigned char mDat)
{
 xWriteCH376Cmd(mCmd);
...
//检查USB存储器错误
/*************************************************************************************************/
unsigned char CH376DiskReqSense(void)
{
 unsigned char s;
 Delay_ms(5);
...
/*************************************************************************************************/
unsigned char CH376FileOpen(unsigned char *name)
{
...
/*************************************************************************************************/
unsigned char CH376FileCreate(unsigned char *name)
{
...
/*************************************************************************************************/
unsigned char CH376DirCreate(unsigned char *name)
{
 CH376SetFileName(name);//设置将要操作的文件的文件名  
 if(name[0] == DEF_SEPAR_CHAR1 || name[0] == DEF_SEPAR_CHAR2) CH376WriteVar32(VAR_CURRENT_CLUST, 0);
...
/*************************************************************************************************/
unsigned char CH376SeparatePath(unsigned char *path)
{
 unsigned char *pName;
...
/*************************************************************************************************/
unsigned char CH376FileOpenDir(unsigned char *PathName,unsigned char StopName)  
{
 unsigned char i,s;
 s = 0;
 i = 1;//跳过有可能的根目录符  
...
/*************************************************************************************************/
unsigned char CH376FileOpenPath(unsigned char *PathName)
{
...
/*************************************************************************************************/
unsigned char CH376FileCreatePath(unsigned char *PathName)
{
 unsigned char s;
 unsigned char Name;
 ...
 if(Name) 
  {  // 是多级目录  
   ...
   if(s != ERR_OPEN_DIR) 
    {//因为是打开上级目录,所以,如果不是成功打开了目录,那么说明有问题  
     if(s == USB_INT_SUCCESS) return(ERR_FOUND_NAME);//中间路径必须是目录名,如果是文件名则出错  
     else if(s == ERR_MISS_FILE) return(ERR_MISS_DIR);//中间路径的某个子目录没有找到,可能是目录名称错误  
     else return(s);//操作出错  
    }
...
/*************************************************************************************************/
unsigned char CH376DirCreatePath(unsigned char *PathName)
{
 unsigned char s;
 unsigned char Name;
 unsigned char ClustBuf[4];
...
/*************************************************************************************************/
unsigned char CH376FileErase(unsigned char *PathName)
{
 unsigned char s;
 if(PathName) 
  {//文件尚未打开  
...
//关闭当前已经打开的文件或者目录(文件夹)
/*************************************************************************************************/
unsigned char CH376FileClose(unsigned char UpdateSz)
{
...
//读取当前文件的目录信息
/*************************************************************************************************/
unsigned char CH376DirInfoRead(void)
{
...
//保存文件的目录信息
/*************************************************************************************************/
unsigned char CH376DirInfoSave(void)
{
...
//以字节为单位移动当前文件指针
/*************************************************************************************************/
unsigned char CH376ByteLocate(unsigned int offset)
{
 xWriteCH376Cmd(CMD4H_BYTE_LOCATE);
 xWriteCH376Data((unsigned char)offset);
 xWriteCH376Data((unsigned char)((unsigned short)offset>>8));
 xWriteCH376Data((unsigned char)(offset>>16));
 xWriteCH376Data((unsigned char)(offset>>24));
 xEndCH376Cmd();
 return(Wait376Interrupt());
}
...
//以字节为单位从当前位置读取数据块
/*************************************************************************************************/
unsigned char CH376ByteRead(unsigned char *buf,unsigned short ReqCount,unsigned short *RealCount)
{
 unsigned char s;
 xWriteCH376Cmd(CMD2H_BYTE_READ);
 xWriteCH376Data((unsigned char)ReqCount);
 xWriteCH376Data((unsigned char)(ReqCount>>8));
 xEndCH376Cmd();
...
//以字节为单位向当前位置写入数据块
/*************************************************************************************************/
unsigned char CH376ByteWrite(unsigned char *buf,unsigned short ReqCount,unsigned short *RealCount)
{
 unsigned char s;
...
//查询磁盘物理容量,扇区数
/*************************************************************************************************/
unsigned char CH376DiskCapacity(unsigned int *DiskCap)
{
 unsigned char s;
...
//查询磁盘剩余空间信息,扇区数
/*************************************************************************************************/
unsigned char CH376DiskQuery(unsigned int *DiskFre)
{
 unsigned char s;
 unsigned char c0,c1,c2,c3;
 ...
 if(s == USB_INT_SUCCESS) 
  {//参考CH376INC.H文件中CH376_CMD_DATA结构的DiskQuery  
   ...
   c2 = xReadCH376Data();
   c3 = xReadCH376Data();
   *DiskFre = c0 | (unsigned short)c1 << 8 | (unsigned int)c2 << 16 | (unsigned int)c3 << 24;
...
//以扇区为单位移动当前文件指针
/*************************************************************************************************/
unsigned char CH376SecLocate(unsigned int offset)
{
...
/*************************************************************************************************/
unsigned char CH376DiskReadSec(unsigned char *buf,unsigned int iLbaStart,unsigned char iSectorCount)  
{
 unsigned char  s, err;
 ...
 for(err = 0; err != 3; ++ err) 
  {//出错重试  
   ...
   xWriteCH376Data((unsigned char)iLbaStart);//LBA的最低8位  
   xWriteCH376Data((unsigned char)((unsigned short)iLbaStart >> 8));
   xWriteCH376Data((unsigned char)(iLbaStart >> 16 ));
   xWriteCH376Data((unsigned char)(iLbaStart >> 24 ));//LBA的最高8位  
...
/*************************************************************************************************/
unsigned char CH376DiskWriteSec(unsigned char *buf,unsigned int iLbaStart,unsigned char iSectorCount)  
{
 unsigned char  s, err;
 unsigned short mBlockCount;
 for(err = 0; err != 3; ++ err) 
...
/*************************************************************************************************/
unsigned char CH376SecRead(unsigned char *buf,unsigned char ReqCount,unsigned char *RealCount)
{
 unsigned char s;
 unsigned char cnt;
 unsigned int  StaSec;
 unsigned int  fsz, fofs;
 if(RealCount) *RealCount = 0;
 do 
  {
   ...
   if(cnt == 0x41) 
    {
     ...
     xReadCH376Data();
     fsz = xReadCH376Data();
     fsz |= (unsigned short)(xReadCH376Data()) << 8;
     cnt = xReadCH376Data();
     fsz |= (unsigned int)cnt << 16;
     xWriteCH376Cmd(CMD14_READ_VAR32);
     xWriteCH376Data(VAR_CURRENT_OFFSET);
     xReadCH376Data();
     fofs = xReadCH376Data();
     fofs |= (unsigned short)(xReadCH376Data()) << 8;
     fofs |= (unsigned int)(xReadCH376Data()) << 16;
...
/*************************************************************************************************/
unsigned char CH376SecWrite(unsigned char *buf,unsigned char ReqCount,unsigned char *RealCount)
{
 unsigned char s;
 unsigned char cnt;
 unsigned int  StaSec;
...
//长文件名专用的字节写子程序
/*************************************************************************************************/
unsigned char CH376LongNameWrite(unsigned char *buf,unsigned short ReqCount)
{
 unsigned char s, c;
...
/*************************************************************************************************/
unsigned char CH376CheckNameSum(unsigned char *DirName)
{
...
/*************************************************************************************************/
unsigned char CH376LocateInUpDir(unsigned char *PathName)
{
 unsigned char s;
 xWriteCH376Cmd(CMD14_READ_VAR32);
 xWriteCH376Data(VAR_FAT_DIR_LBA);//当前文件目录信息所在的扇区LBA地  
...
/*************************************************************************************************/
unsigned char CH376GetLongName(unsigned char *PathName,unsigned char *LongName)
{
 unsigned char  s;
 unsigned short NameCount;//长文件名字节计数  
 s = CH376FileOpenPath(PathName);//打开多级目录下的文件或者目录  
 ...
 while(1) 
  {
   ...
   for(s = 1;s < sizeof(FAT_DIR_INFO);s += 2) 
    {//收集长文件名,长文件名的字符在磁盘上UNICODE用小端方式存放  
   ...
   if(GlobalBuf[0] & 0x40) 
    {//长文件名目录信息块结束  
     if(s >= sizeof(FAT_DIR_INFO)) *(unsigned short *)(&LongName[NameCount]) = 0x0000;//尚未收集到长文件名的结束符,则强制结束  
...
/*************************************************************************************************/
unsigned char CH376CreateLongName(unsigned char *PathName,unsigned char *LongName)
{
 unsigned char  s,i;
 unsigned char  DirBlockCnt;//长文件名占用文件目录结构的个数  
 unsigned short count;    	//临时变量,用于计数,用于字节读文件方式下实际读取的字节数  
 unsigned short NameCount;    //长文件名字节计数  
 unsigned int   NewFileLoc;    //当前文件目录信息在上级目录中的起始位置,偏移地址  
 for(count = 0;count < LONG_NAME_BUF_LEN;count += 2) if(*(unsigned short *)(&LongName[count]) == 0) break;  // 到结束位置  
